...
Rollout configuration changes to your primary gerrit instance (`gerrit.config`)
```
[noteDb "changes"]
...
Rollout configuration changes to your secondary gerrit instance (`gerrit.config`)
...
Once both primary and secondary gerrit have been rolled out you can set them
back to RW mode.
...
The migration will run for a period of time that depends of your system
...
As for step 2, you can leave Gerrit with this configuration for a period of time
...
will not be available in ReviewDB and will be lost.
   ...
   4.2. Take a snapshot Git repos, review DB, caches indexes
   4.3. Roll out the following configuration to both primary and secondary gerrit.
   ...
   [noteDb "changes"]
      ...
      trial = false
   ```
   4.4. Put primary and secondary gerrit in Read/Write mode
...
At any stage of the migration a rollback can be performed by updating
`gerrit.config` on both primary and secondary node as follows:
