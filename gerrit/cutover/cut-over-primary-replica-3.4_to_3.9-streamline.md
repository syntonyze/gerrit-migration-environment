...
* Verify that the OpenSSH key algorithm matches the one associated with the public key.
Throw a new specific InvalidKeyAlgorithmException if the two algorithms do not match.
...
* Download new Gerrit 3.9.4 war file from the
  [Gerrit Code Review 3.9 official website](https://gerrit-releases.storage.googleapis.com/gerrit-3.9.4.war).
...
*NOTE*: To speedup the online reindex, it is possible to increase the number of
allocated threads by adjusting the [sshd.batchThreads](https://gerrit-review.googlesource.com/Documentation/config-gerrit.html#sshd.batchthreads) parameter.
Make sure to to be re-establish the original value and Gerrit restarted once the
online reindexing will be over.
