...
7. Run project reindex offline. Consider increaring memory configuration (`-Xmx<>`) if
...
the project's `refs/meta/config` is unchanged and therefore steps `#8` and `#9` aren't
needed anymore.
