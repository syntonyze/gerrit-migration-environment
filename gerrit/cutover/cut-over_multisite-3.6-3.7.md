...
* SSH queries doesn't show commit-message anymore unless `–commit-message` is provided.
...
```
   ...
   *Note*: that you should remove any core plugin you don't want to install
   ...
   ```shell
    ...
    ... using 10 threads ...
    ... (0.136 s) Migrated label configurations of all X projects to schema 185
   ```
...
3. Downgrade the schema version from 185 to 184 as explained [here](https://www.gerritcodereview.com/3.7.html).
...
customers' browsers cached some client side requests done on 3.6. In this case
