...
1.3. Once both primary and secondary gerrit have been rolled out you can set them
   back to RW mode.

*Note*:
You also have control on how many threads to dedicate to the online migration,
by default *1* thread is dedicated.
...
in the `etc/gerrit.config` file, for example:
...
to the gerrit instance, in the `etc/notedb.config` configuration file.
...
will not be available in ReviewDB and will be lost.
 ...
 4.3. Roll out the following configuration to primary gerrit, in the
...
5.3. Roll out the above configuration to secondary gerrit too, in the
`etc/notedb.config` file.
...
5.4. Put primary and secondary gerrit in Read/Write mode
