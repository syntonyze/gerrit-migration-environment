...
* caches: persistent H2 database files stored in the `<gerrit_dir>/cache` directory
* indexes: Lucene index files stored in the `<gerrit_dir>/index` directory
...
* Upgrade high-availability plugin to the [latest version](./link)

* Disable draft changes, Private and patch-sets and notify a deadline for the people to
...
3a. Simple with longer Read Only window:
  * Put the system in Read Only (`touch <gerrit_dir>/etc/gerrit.readonly` on both gerrit-1 and gerrit-2)
  * Take a snapshot of Git repositories, review DB, caches, indexes
  * Put the system back in Read Write (`rm <gerrit_dir>/etc/gerrit.readonly` on both gerrit-1 and gerrit-2)
3b. Complex with short Read Only window:
  * Rsync the git repositories prior to starting the migration to a backup storage
  * Put system in Read Only (`touch <gerrit_dir>/etc/gerrit.readonly` on both gerrit-1 and gerrit-2)
...
4. Take note of the time the node has been stopped (we will refer it as
  `migration-start-time-gerrit-2`). It will be needed in case of [rollback](./Rollback strategy).
5. Upgrade plugins, lib and war file on gerrit-2
6. Run gerrit init on gerrit-2 `java -jar gerrit-2.15.22.war init -d <gerrit_dir> --no-auto-start`:
...
[2021-05-12 19:33:23,890] [Reindex groups v2-v4] WARN  com.google.gerrit.server.index.OnlineReindexer : Error activating new groups schema version 4`
  * Further check is the following, where `groups_0004`, `accounts_0007` and `changes_0048` need to be marked as `ready = true`:
...
11. Mark node as healthy. Log into gerrit-2 and run
  `ssh -p 29418 gerritadmin@localhost gerrit plugin enable healthcheck`
12. Route traffic to gerrit-2 making it the primary node. Set gerrit-2 as primary
...
This will allow a [quick rollback](./Rollback strategy) to Gerrit v2.14 if needed.
...
It will be needed in case of [rollback](./Rollback strategy).
16. Upgrade plugins, lib and war file on gerrit-1
17. Copy indexes over from gerrit-2 node to gerrit-1. All the `<gerrit_dir>/index` directory
need to be copied over
18. The HA plugin keeps the last update timestamp for each index in the following files:
`<gerrit_dir>/data/high-availability/group`
`<gerrit_dir>/data/high-availability/account`
`<gerrit_dir>/data/high-availability/change`
