...
* Upgrade of Gerrit Code Review to [v2.14.22](https://www.gerritcodereview.com/2.14.html), repeat the
  steps below for gerrit-1 first and then gerrit-2 afterwards:

  - Make Gerrit unhealthy (unload the `healthcheck` plugin)
  - Shutdown Gerrit
  - Replace gerrit.war
  ...
  - Startup Gerrit
  - Rename the `$GERRIT_SITE/plugins/healthcheck.jar.disabled` to `$GERRIT_SITE/plugins/healthcheck.jar`
